...
The statechain is a data structure that is used to prove that the ownership of each UTXO is unique. This proof is verified by the new owner, and is updated via the SE. The statechain contains the transaction history of each UTXO managed by the SE (which is a requirement to prove uniqueness). The uniqueness of the statechain for a specific SE is guaranteed via the Mainstay protocol that utilise Bitcoin's global state (resulting in a verifiable *proof of publication* for each ownership change). 
...
The life-cycle of a P2PKH depost into the statechain, transfer and withdrawal is summarised as follows:
...
3. Owner 1 can verifiably transfer ownership of the UTXO to a new party (Owner 2) via a key update procedure that overwrites the private key share of SE that invalidates the Owner 1 private key and *activates* the Owner 2 private key share. 
4. Owner 2 and the SE cooperatively sign a backup *kickoff* transaction spending the kickoff transaction to an address controlled by Owner 2 which can be confirmed after an `nSequence` locktime, which is shorted (by an accepted confirmation interval) than the previous owner. 
...
As described above, double-spending of the UTXO (by a corrupt SE) is prevented by a proof-of-uniqueness from the Mainstay protocol. Each unique transfer of the UTXO between owners is recorded on the SE statechain. 
...
In addition, a public key encryption scheme is required blinded key information sent between parties. This should be compatible with the EC keys used for signatures, and ECIES will be used. The notation for the use of ECIES operations is as follows: `Enc(m,K)` denotes the encryption of message `m` with public key `K = k.G` and `Dec(m,k)` denotes the decryption of message `m` using private key `k`. 

All transactions are created and signed using segregated witness, which enables input transaction IDs to be determined before signing and prevents their maeliability. 
...
> The above key sharing scheme is the same as that used in all 2P ECDSA protocols. The key generation routines of an existing 2P ECDSA implementation can be used in place of the above steps (which may include additional verification steps). 

5. Owner 1 creates a funding transaction (`Tx0`) to pay an amount `A` to the address corresponding to `P` (but doesn't sign it) [this transaction may also have an output for a fee `F` paid to the SE]. This defines the UTXO `TxID:n` (the outpoint), which is send to the SE.
...
10. SE receives `Tx1` and verifies the `nSequence` field. Owner 1 and the SE then sign `Tx1` with shared key (`P`) via 2P ECDSA, which Owner 1 then saves. 
...
12. The SE then adds the UTXO outpoint with public key `O1` to the *statechain* which is then attested to Bitcoin via the Mainstay protocol. 
...
1. Owner 2 generates a private keys `o2` (the new owner UTXO private key share). They then compute the public key share `O2 = o2.G`. 
2. `O2` then represents the Owner 2 'address' and is sent to Owner 1 (or published) in order for them to 'send' the ownership. 
3. Owner 1 then requests that the SE facilitate a transfer to Owner 2 (and that the new owner can be authenticated with `O2`). 
4. SE generates a random key `x1` and encrypts it with the Owner 2 public key: `Enc(x1,O1)`
5. `Enc(x1,O1)` is sent to Owner 1 who decrypts it with `o1` to learn `x1`: `Dec(x1,o1)`
6. Owner 1 then computes `o1*x1` and encrypts it with the Owner 2 public key (address): `Enc(o1*x1,O2)`

> The steps 3-6 only require interaction between the SE and owner 1, and can be performed at any time before the involvement of Owner 2 is required. 

7. Owner 1 sends Owner 2 a message containing their public key `O1` and `Enc(o1*x1,O2)`. 

> At this point the Owner 1 has sent all the information required to complete the transfer to Owner 2 and is no longer involved in the protocol. Owner 1 can then complete the transfer with the SE at any time. 

8. Owner 2 decrypts `Dec(o1*x1,o2)` and then computes `o1*x1*o2_inv` where `o2_inv` is the modular inverse of the private key `o2`. 
9. Owner 2 then encrypts `Enc(o1*x1*o2_inv,SE)`, signs it with `O2` and sends it to the SE. 
10. The SE authenticates and decrypts this to learn `o1*x1*o2_inv`: `Dec(o1*x1*o2_inv,se)`
11. The SE then multiplies this product by `x1_inv*s1` (where `x1_inv` the modular inverse of `x1`) to compute `s2 = o1*o2_inv*s1`
12. The SE then deletes the key share `s1`. If the SE operations are run in a secure enclave, a remote attestation of this can be sent to Owner 2. 

> `s2` and `o2` are now key the private key shares of `P = s2*o2.G` which remains unchanged (i.e. `s2*o2 = s1*o1`), without anyone having learnt the full private key. Provided the SE deletes `s1`, then there is no way anyone but the current owner (`O2`) can spend the output. 

13. The SE sends Owner 2 `S2 = s2.G` who verifies that `o2.S2 = P`
14. Owner 2 generates a backup private key `b2` and send `B2 = b2.G` to the SE.
15. The SE sends Owner 2 the signed kick-off transaction `TxK`
16. Owner 2 creates a new *backup transaction* (`Tx2`) that pays the `P` output of `TxK` to `B2`, and sets the `nSequence` to the relative locktime `t0 - (n-1)*c` (according to BIP68) where `c` is the confirmation interval and `n` is the owner number (i.e. 2). 
17. The SE recieves `Tx1` and verifies the `nSequence` field corresponds to `t0 - (n-1)*c`. Owner 1 and the SE then sign `Tx2` with shared key (`P`) via 2P ECDSA, which Owner 1 then saves. 
...
3. The owner signs the string: `TxID:n:close` with their key `O2` and sends it to the SE. 
4. SE and the owner sign `TxW`
3. The fully signed `TxW` is then broadcast and confirmed. 
4. The SE sends the close string statechain, to verifiably close the UTXO chain of ownership. 
...
3. The SE then generates a private key: `s1` (the SE private key share), calculates the corresponding public key and sends it to Owner 1: `S1 = s1.G`
4. Both SE and Owner 1 then multiply the public keys they receive by their own private key shares to obtain the same shared public key `P` (which corresponds to a shared private key of `p = o1*s1`): `P = o1.(s1.G) = s1.(o1.G)`
5. Owner 1 and the DLC counterparty create the DLC funding transaction `Tx0` (with supplied inputs) to pay an amount `A` to a 2-of-2 multisig (one public key `P` and the other public key belonging to the counterpary's - `C`. This defines the opening UTXO `TxID` and `n` (the outpoint). 
