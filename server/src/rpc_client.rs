...
impl RPCClient {
	...
	pub fn make_rpc_call(&self, method: &str, params: &[&str], may_fail: bool) -> Result<serde_json::Value, ()> {
		...
		if let Ok(res) = self.client.request(request.body(hyper::Body::from(req.clone())).unwrap()) {
			if res.status() != hyper::StatusCode::OK {
				...
				if let Ok(body) = res.into_body().map_ok(|b| b.to_vec()).try_concat() {
