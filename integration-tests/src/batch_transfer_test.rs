...
mod tests {
    ...
    fn test_batch_transfer() {
     ...
     /// Perform batch transfer and interrupt before the transfer finalize stage.
     /// Then attempt wallet recovery.
     #[test]
     #[serial]
     fn test_batch_interrupted_recovery() {
         let _handle = start_server(None, None);
         ...
         dbg!("doing run batch transfer...");
         let (batch_id, _transfer_finalized_datas, commitments, nonces, transfer_sigs) =
             run_batch_transfer(&mut wallets, &swap_map, &funding_txids, &statechain_ids);
         dbg!("finished run batch transfer.");
