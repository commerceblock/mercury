...
mod tests {
    ...
    fn test_double_transfer() {
        ...
        {
            let shared_key = wallets[0].get_shared_key_by_state_chain_id(state_chain_id).unwrap();
        ...
        {
            let shared_key = wallets[1].get_shared_key_by_state_chain_id(state_chain_id).unwrap();
    ...
    fn test_withdraw() {
        ...
        assert!(wallet.get_shared_key_by_state_chain_id(state_chain_id).unwrap().unspent);
        ...
        // Check state chain is updated
        let state_chain = state_entity::api::get_statechain(&wallet.client_shim, state_chain_id).unwrap();
        assert_eq!(state_chain.chain.len(),2);
