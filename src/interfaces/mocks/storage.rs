...
mod tests {
    ...
    fn quick_tests() {
        ...
        let _ = mock_storage.append_to_chain(0, "tomos".to_string());
        assert!(mock_storage.state_chains[0].chain[0] == "tomos");
        assert!(mock_storage.state_chains[0].chain.len() == 1);
        // get_chain_state
        assert!(mock_storage.get_chain(0).unwrap().chain == ["tomos"]);
    }
}
