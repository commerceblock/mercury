...
impl SharedWallet {
    pub fn new(client_shim: &ClientShim, net: &String) -> SharedWallet {
...
//            .filter(|b| b.confirmed > 0)
    ...
    pub fn get_all_addresses(&self) -> Vec<bitcoin::Address> {
        ...
        for n in init..=last_pos {
            ...
            let bitcoin_address = Self::to_p2wpkh_address(&mk, self.get_bitcoin_network());
