...
use shared_lib::structs::{DepositMsg1, DepositMsg2, PrepareSignTxMsg, Protocol, UserID, StatechainID, StateEntityFeeInfoAPI};
use shared_lib::util::{tx_backup_build, tx_funding_build, FEE, transaction_serialise};

use super::api::{get_smt_proof, get_smt_root, get_statechain_fee_info, pod_token_init, pod_token_verify};};
...
pub fn deposit(
    ...
    let (inputs, addrs, amounts) =
        wallet.coin_selection_greedy(&(amount + withdraw_fee + FEE*se_fee_info.backup_fee_rate))?;
    ...
    let change_amount = amounts.iter().sum::<u64>() - amount - withdraw_fee - FEE*se_fee_info.backup_fee_rate;
